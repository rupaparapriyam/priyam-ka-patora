...
is_torso_tshirt = alpha & (np.arange(H)[:, None] >= 265) & (np.arange(H)[:, None] < 555) & (np.arange(W)[None, :] >= 60) & (np.arange(W)[None, :] <= 288)
is_upper_sleeves = alpha & (np.arange(H)[:, None] >= 285) & (np.arange(H)[:, None] <= 440) & ((np.arange(W)[None, :] <= 95) | (np.arange(W)[None, :] >= 252))
is_full_arms = alpha & (np.arange(H)[:, None] >= 285) & (np.arange(H)[:, None] <= 690) & ((np.arange(W)[None, :] <= 95) | (np.arange(W)[None, :] >= 252))
is_lower_chinos = alpha & (np.arange(H)[:, None] >= 555) & (np.arange(H)[:, None] <= 915)
is_feet = alpha & (np.arange(H)[:, None] > 915)

# Calculate smooth 32-bit luminance for cloth folds
...
# Rolled Sleeve Cuffs
for y in range(432, 442):
    for x in range(W):
        if is_upper_sleeves[y, x]:
            casual[y, x] = [55, 34, 22, 255]
...
# Cream / Beige Chinos
for y, x in zip(*np.where(is_lower_chinos)):
    b = lum_m[y, x]
    casual[y, x] = [int(180 + b * 45), int(170 + b * 45), int(155 + b * 45), 255]
    if abs(x - 174) <= 1 and y > 670:
        casual[y, x] = [140, 130, 118, 255] # Inseam crease
...
# Obsidian Matte Black Hoodie
for y, x in zip(*np.where(is_torso_tshirt | is_full_arms)):
    b = lum_m[y, x]
...
is_match_shorts = alpha & (np.arange(H)[:, None] >= 555) & (np.arange(H)[:, None] <= 705)
for y, x in zip(*np.where(is_match_shorts)):
    b = lum_m[y, x]
    cr7[y, x] = [int(215 + b * 40), int(220 + b * 35), int(230 + b * 25), 255]
    if x in [95, 253]: cr7[y, x] = [230, 185, 25, 255]

# White Socks with Gold Bands
is_socks = alpha & (np.arange(H)[:, None] >= 770) & (np.arange(H)[:, None] <= 915)
...
# Red Sleeves with Black Gloves
for y, x in zip(*np.where(is_full_arms)):
    if y >= 560: f1[y, x] = [20, 20, 24, 255]
    else: f1[y, x] = [215, 25, 32, 255]

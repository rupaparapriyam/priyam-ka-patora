...
**Note (2026-08-30): actual code does NOT live inside the vault.** Once an idea moves from research into a real app/prototype, its code lives one level up, at the project root — a sibling of this `Vault/` folder, e.g. `../ecommerce-hub-app/`, `../pathlab-ops-app/` (both live at `/Users/priyamrupapara/developer/A priyam ka patora/<app-name>/`). This keeps the "exactly one file per idea folder" rule in this vault fully intact — code is deliberately kept out, same reasoning as `_project-briefs/` living outside the vault. Each idea's `.md` file references its code folder's path in plain text/code formatting (not a wikilink — it's outside the vault, Obsidian can't resolve a link across that boundary), and each code folder's own `README.md` links back to its vault `.md` file with a relative path. A code folder is tracked by the same outer git repo (the one this vault also lives in) — don't leave a separate, standalone `.git` inside it; if one already exists, rename it aside (e.g. `.git-standalone-history-backup`) rather than deleting it, so history isn't lost.
...
- **2026-08-30**: code for an idea lives at the project root (sibling of `Vault/`, e.g. `ecommerce-hub-app/`, `pathlab-ops-app/`) instead of scattered under `~/developer/` in unrelated folders — but explicitly NOT inside the vault itself, keeping "one file per idea folder" intact. Also renamed this folder from the confusing nested `A priyam ka patora/A priyam ka patora/` to `A priyam ka patora/Vault/`.
